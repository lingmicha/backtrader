...
class TurtleTrend(bt.Strategy):
    params = (
        # ('index_sma', 90),
        ('breakout_window', 28800 ),  # 28800 min - 20day
        ('exit_window', 14400 ),  # 14400 min - 10day
        ('atr', 28800 ),  # 28800 min - 20day
        ('max_units', 4),
        ('debug', True),
        ('live_feed', False),
        ('live_trading', False),
    ...
    def next(self):
        ...
        if self.p.live_trading and (not self.live_data ):
            return # prevent trading with delayed data
...
def run_backtest():
    ...
    fromdate = datetime.strptime('2020-09-01', '%Y-%m-%d')
...
def run_live_trading():
    ...
    # TODO: DATA0 Must have the earilest start datetime
    fromdate = datetime.strptime('2020-03-01', '%Y-%m-%d')
    todate = datetime.strptime('2022-03-30', '%Y-%m-%d')
